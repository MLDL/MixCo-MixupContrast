...
class MixCo3(nn.Module):
    ...
    def forward(self, im_q, im_k):
        ...
        im_q, q_idx1, q_idx2 = self.data_mixer(im_q)
        im_k, k_idx1, k_idx2 = self.data_mixer(im_k)
        ...
        # positive logits: Nx1
        l_pos1 = torch.einsum('nc,nc->n', [q[q_idx1], k[k_idx1]]).unsqueeze(-1)
        l_pos2 = torch.einsum('nc,nc->n', [q[q_idx2], k[k_idx2]]).unsqueeze(-1)

        # negative logits: NxK
        l_neg1 = torch.einsum('nc,ck->nk', [q[q_idx1], self.queue.clone().detach()])
        l_neg2 = torch.einsum('nc,ck->nk', [q[q_idx2], self.queue.clone().detach()])
